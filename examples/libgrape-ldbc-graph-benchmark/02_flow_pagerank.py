...
class Pagerank(FlowSpec):
    ...
    def multinode(self):
        ...
        current.mpi.exec(
            ...
            program_args=[
                "--application", "pagerank",             # Application - counting the number and quality of links to a page
                "--vfile", "%s/p2p-31.v" % dataset_path, # Vertex data
                "--efile", "%s/p2p-31.e" % dataset_path, # Edge data
                "--pr_mr", self.max_rounds,
                "--pr_d", self.damping_factor,
                "--out_prefix", "./output_pagerank"
            ]
        )
